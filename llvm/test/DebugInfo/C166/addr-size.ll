; A C166 address in the debug information is four bytes and carries the whole
; physical address, which is not what a pointer in the program does.  A pointer
; is a 16 bit offset that the data page pointers or CSP place somewhere in the
; 16 MByte the part addresses, so two bytes cannot say where anything is: on
; this part, whose Flash is at C0'0000H, every function would be described as
; living in segment 0.  So the address size is four and the relocation is
; ABS32, the full value, rather than the SOF16 an instruction operand takes.
;
; RUN: llc -filetype=obj -mtriple=c166 < %s -o %t.o
; RUN: llvm-dwarfdump --debug-info %t.o 2>&1 | FileCheck %s
; RUN: llvm-readobj -r %t.o | FileCheck %s --check-prefix=RELOC

; CHECK: addr_size = 0x04
; CHECK: DW_AT_name ("add")

; The addresses in .debug_addr are what DW_AT_low_pc is written in terms of,
; and each has to be able to hold a segment.
; RELOC: Section {{.*}} .rela.debug_addr {
; RELOC-NEXT: 0x{{[0-9A-F]*}} R_C166_ABS32 .text 0x0
; RELOC-NEXT: 0x{{[0-9A-F]*}} R_C166_ABS32 .text 0x
; RELOC-NEXT: }
